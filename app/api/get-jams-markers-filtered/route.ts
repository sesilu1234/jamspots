...
export async function GET(req: Request) {
  ...
  if (dateOptions && dateOptions !== 'all' && userDate) {
    ...
    if (dateOptions === 'week') {
      ...
      query = query.or(`periodicity.eq.weekly,dates.overlaps.{${rangeDates}}`);
      ...
      const weekDay = customDate.getDay(); // 0 = domingo, 1 = lunes, ...

      query = query.or(
        `dayOfWeek.eq.${weekDay},dates.overlaps.{${[customDate]}}`,
      );
    }
  }
