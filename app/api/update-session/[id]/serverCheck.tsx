...
export function validateJam(input: any) {
  ...
  else if (input.jam_title.length > 50)
    errors.jam_title = "Jam name too long";
  ...
  if (!Array.isArray(input.images) || input.images.length < 2)
    errors.images = "At least 2 images required";
  else if (input.images.length > 4)
    errors.images = "Max 4 images allowed";

  // styles
  if (!Array.isArray(input.styles) || input.styles.length < 1)
    errors.styles = "Select at least one style";
  else if (input.styles.length > 3)
    errors.styles = "Max 3 styles allowed";
...
if (typeof input.time_start !== "string") {
  ...
  if (!input.location_coords) {
    ...
    if (typeof lat !== "number" || typeof lng !== "number") errors.coo = "Coordinates must be a number";
    else if (lat < -90 || lat > 90 || lng < -180 || lng > 180) errors.coorange = "Latitude out of range";
