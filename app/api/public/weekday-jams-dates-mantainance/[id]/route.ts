...
export async function POST(req: Request) {
  try {
    ...
    const { data: weeklyJams, error: profileError } = await supabaseAdmin
      .from('sessions')
      .select('id, dayOfWeek, timezone, time_start')
      .eq('periodicity', 'weekly');

    if (profileError) throw profileError;
    if (!weeklyJams) return NextResponse.json({ success: true, message: 'No jams found' });
    ...
    const getWeeklyDatesUTC = (jam: JamSession): string[] => {
      ...
      if (now.weekday <= targetDayNumber) {
        ...
        currentPointer = now.plus({ weeks: 1 }).set({ weekday: targetDayNumber });
      }

      // Loop exactly 15 times
      while (schedule.length < 15) {
        const dateStr = currentPointer.toISODate();
        const eventTime = DateTime.fromISO(`${dateStr}T${jam.time_start}`, { zone: jam.timezone });
